...
class ACFMDataset(TrafficStateGridDataset, TrafficStateCPTDataset):
    ...
    def _get_external_array(
        ...
        if ext_time:
            vecs_timestamp = timestamp2array(
                ...
                24)  # len(timestamp_list) * dim
        else:
            vecs_timestamp = timestamp2vec_origin(
                timestamp_list)  # len(timestamp_list) * dim
    ...
    def _load_ext_data(self, ts_x, ts_y):
        ...
        for ts in ts_x:
            ext_x.append(
                self._get_external_array(
                    ts,
                    ext_data,
                    ext_time=self.external_time))
    ...
    def get_data_feature(self):
        ...
        """
        lp = self.len_period * \
            (self.pad_forward_period + self.pad_back_period + 1)
        lt = self.len_trend * (self.pad_forward_trend +
                               self.pad_back_trend + 1)
        return {
            "scaler": self.scaler,
            "adj_mx": self.adj_mx,
