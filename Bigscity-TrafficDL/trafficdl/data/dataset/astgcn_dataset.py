...
class ASTGCNDataset(TrafficStatePointDataset):
    ...
    def __init__(self, config):
        ...
        self.interval_period = self.config.get(
            'interval_period', 1)  # period的长度/天
        self.interval_trend = self.config.get(
            'interval_trend', 7)    # trend的长度/天
    ...
    def _get_sample_indices(self, data_sequence, label_start_idx):
        ...
        target = data_sequence[label_start_idx: label_start_idx +
                               self.output_window]
    ...
    def _generate_input_data(self, df):
        ...
        for idx in range(df.shape[0]):
            ...
            if (sample[0] is None) and (
                    sample[1] is None) and (sample[2] is None):
                continue
            trend_sample, period_sample, closeness_sample, target = sample
            if self.len_trend > 0:
                trend_sample = np.expand_dims(
                    trend_sample, axis=0)  # (1,Tw,N,F)
                trend_samples.append(trend_sample)
            if self.len_period > 0:
                period_sample = np.expand_dims(
                    period_sample, axis=0)    # (1,Td,N,F)
                period_samples.append(period_sample)
            if self.len_closeness > 0:
                closeness_sample = np.expand_dims(
                    closeness_sample, axis=0)  # (1,Th,N,F)
        ...
        if len(closeness_samples) > 0:
            closeness_samples = np.concatenate(
                closeness_samples, axis=0)  # (num_samples,Th,N,F)
        ...
        if len(period_samples) > 0:
            period_samples = np.concatenate(
                period_samples, axis=0)    # (num_samples,Td,N,F)
        ...
        if len(trend_samples) > 0:
            trend_samples = np.concatenate(
                trend_samples, axis=0)  # (num_samples,Tw,N,F)
