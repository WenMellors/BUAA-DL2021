...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.weight_col = self.data_config.get(
            'info', {}).get('weight_col', '')
        self.data_col = self.data_config.get('info', {}).get('data_col', '')
        self.ext_col = self.data_config.get('info', {}).get('ext_col', '')
        self.geo_file = self.data_config.get(
            'info', {}).get(
            'geo_file', self.dataset)
        self.rel_file = self.data_config.get(
            'info', {}).get(
            'rel_file', self.dataset)
        self.data_files = self.data_config.get(
            'info', {}).get(
            'data_files', self.dataset)
        self.ext_file = self.data_config.get(
            'info', {}).get(
            'ext_file', self.dataset)
        # 初始化
        self.data = None
    ...
    def _load_geo(self):
        ...
        self._logger.info("Loaded file " + self.geo_file +
                          '.geo' + ', num_nodes=' + str(len(self.geo_ids)))
    ...
    def _load_rel(self):
        ...
        if self.weight_col != '':  # 根据weight_col确认权重列
            if isinstance(self.weight_col, list):
                if len(self.weight_col) != 1:
                    raise ValueError(
                        '`weight_col` parameter must be only one column!')
        ...
        for row in self.distance_df.values:
            if row[0] not in self.geo_to_ind or row[1] not in self.geo_to_ind:
                continue
            self.adj_mx[self.geo_to_ind[row[0]],
                        self.geo_to_ind[row[1]]] = row[2]
        self._logger.info("Loaded file " + self.rel_file +
                          '.rel, shape=' + str(self.adj_mx.shape))
        # 计算权重
    ...
    def _load_grid_rel(self):
        ...
        self._logger.info("Generate grid rel file, shape=" +
                          str(self.adj_mx.shape))
    ...
    def _load_dyna(self, filename):
        ...
        raise NotImplementedError(
            'Please implement the function `_load_dyna()`.')
    ...
    def _load_dyna_3d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(dynafile['time'][:int(
            dynafile.shape[0] / len(self.geo_ids))])
        self.idx_of_timesolts = dict()
        if not dynafile['time'].isna().any():  # 时间没有空值
            self.timesolts = list(map(lambda x: x.replace(
                'T', ' ').replace('Z', ''), self.timesolts))
        ...
        # (len_time, len(self.geo_ids), feature_dim)
        data = data.swapaxes(0, 1)
        self._logger.info("Loaded file " + filename +
                          '.dyna' + ', shape=' + str(data.shape))
        return data
    ...
    def _load_grid_3d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(gridfile['time'][:int(
            gridfile.shape[0] / len(self.geo_ids))])
        self.idx_of_timesolts = dict()
    ...
    def _load_grid_od_4d(self, filename):
        ...
        # 求时间序列
        self.timesolts = list(gridodfile['time'][:int(
            gridodfile.shape[0] / len(self.geo_ids) / len(self.geo_ids))])
        self.idx_of_timesolts = dict()
        ...
        data = np.zeros(
            (len(
                self.geo_ids), len(
                self.geo_ids), len_time, feature_dim))
        for oi in range(self.len_row):
            for oj in range(self.len_column):
                origin_index = (oi * self.len_column + oj) * \
                    len_time * len(self.geo_ids)  # 每个起点占据len_t*n行
                for di in range(self.len_row):
                    for dj in range(self.len_column):
                        destination_index = (
                            di * self.len_column + dj) * len_time  # 每个终点占据len_t行
    ...
    def _load_ext(self):
        ...
        if not extfile['time'].isna().any():  # 时间没有空值
            self.ext_timesolts = list(map(lambda x: x.replace(
                'T', ' ').replace('Z', ''), self.ext_timesolts))
            self.ext_timesolts = np.array(
                self.ext_timesolts, dtype='datetime64[ns]')
        ...
        self._logger.info("Loaded file " + self.ext_file +
                          '.ext' + ', shape=' + str(df.shape))
        return df
    ...
    def _add_external_information_3d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            time_ind = (
                self.timesolts - self.timesolts.astype("datetime64[D]")) / np.timedelta64(1, "D")
            time_in_day = np.tile(
                time_ind, [
    ...
    def _add_external_information_4d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            time_in_day = np.tile(
                time_ind, [
                    1, len_row, len_column, 1]).transpose(
                (3, 1, 2, 0))
            data_list.append(time_in_day)
        if self.add_day_in_week and not is_time_nan:
            dayofweek = []
    ...
    def _add_external_information_6d(self, df, ext_data=None):
        ...
        if self.add_time_in_day and not is_time_nan:
            ...
            time_in_day = np.tile(
                time_ind, [
                    1, len_row, len_column, len_row, len_column, 1]). transpose(
                (5, 1, 2, 3, 4, 0))
            data_list.append(time_in_day)
        if self.add_day_in_week and not is_time_nan:
            dayofweek = []
    ...
    def _generate_input_data(self, df):
        ...
        x_offsets = np.sort(np.concatenate(
            (np.arange(-self.input_window + 1, 1, 1),)))
    ...
    def _generate_data(self):
        ...
        if self.load_external and os.path.exists(
                self.data_path + self.ext_file + '.ext'):  # 外部数据集
        ...
        self._logger.info("x shape: " + str(x.shape) +
                          ", y shape: " + str(y.shape))
        return x, y

    # 可以将一个数据集按照一定的比例划分为不同的数据集类型
    def _split_train_val_test(self, x, y):
        ...
        self._logger.info("train\t" +
                          "x: " +
                          str(x_train.shape) +
                          "y: " +
                          str(y_train.shape))
        self._logger.info("eval\t" +
                          "x: " +
                          str(x_val.shape) +
                          "y: " +
                          str(y_val.shape))
    ...
    def _get_scalar(self, x_train, y_train):
        ...
        if self.scaler_type == "normal":
            scaler = NormalScaler(maxx=max(
                x_train[..., :self.output_dim].max(), y_train[..., :self.output_dim].max()))
            ...
            self._logger.info('StandardScaler mean: ' +
                              str(scaler.mean) + ', std: ' + str(scaler.std))
            ...
            self._logger.info('MinMax01Scaler max: ' +
                              str(scaler.max) + ', min: ' + str(scaler.min))
        elif self.scaler_type == "minmax11":
            scaler = MinMax11Scaler(
                maxx=max(x_train[..., :self.output_dim].max(), y_train[..., :self.output_dim].max()),
                minn=min(x_train[..., :self.output_dim].min(), y_train[..., :self.output_dim].min()))
            self._logger.info('MinMax11Scaler max: ' +
                              str(scaler.max) + ', min: ' + str(scaler.min))
    ...
    def get_data_feature(self):
        ...
        """
        raise NotImplementedError(
            'Please implement the function `get_data_feature()`.')
