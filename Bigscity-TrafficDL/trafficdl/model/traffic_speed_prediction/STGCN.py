...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.act == "GLU":
            ...
            # Q]
            return (x_conv[:, :self.c_out, :, :] + x_in) * \
                torch.sigmoid(x_conv[:, self.c_out:, :, :])
    ...
    def __init__(self, ks, c_in, c_out, lk, device):
        ...
        self.theta = nn.Parameter(
            torch.FloatTensor(
                c_in,
                c_out,
                ks).to(device))  # kernel: C_in*C_out*ks
...
class STGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.st_conv2 = STConvBlock(
            ...
            self.Lk,
            self.device)
        self.output = OutputLayer(self.blocks[1][2], self.input_window - len(
            self.blocks) * 2 * (self.Kt - 1), self.num_nodes, self.output_dim)

    def forward(self, batch):
        x = batch['X']  # (batch_size, input_length, num_nodes, feature_dim)
        # (batch_size, feature_dim, input_length, num_nodes)
        x = x.permute(0, 3, 1, 2)
        ...
        outputs = self.output(x_st2)
        # (batch_size, output_length(1), num_nodes, output_dim(1))
    ...
    def predict(self, batch):
        ...
        for i in range(output_length):
            batch_tmp = {'X': x_}
            # (batch_size, 1(output_length), num_nodes, 1(feature_dim))
        ...
        # concat at time_length, y_preds.shape=y.shape
