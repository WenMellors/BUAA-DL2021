...
class TrafficStateCPTDataset(TrafficStateDataset):
    ...
    **数据原始的时间戳不能为空！**
    ...
    def __init__(self, config):
        ...
        self.offset_frame = np.timedelta64(
            60 // int(self.points_per_hour), 'm')
    ...
    def _generate_input_data(self, df):
        ...
        rl_p = [
            range(
                ...
                tday *
                i -
                self.pad_forward_period,
                self.interval_period *
                tday *
                i +
                self.pad_back_period +
                1) for i in range(
                1,
                self.len_period +
                1)]
        ...
        largest_interval = max(
            [k[-1] if len(k) != 0 else 0 for k in offset_mat])
        ...
        for cur_ts in self.timesolts[largest_interval:]:
            ...
            # ts_mat和offset_mat形状相同([[closeness], [period],
            # [trend]])，存储的是具体的时间戳
            ts_mat = \
                [
                    [
            ...
            if not flag:  # 有异常时间戳则进入下一次循环
                continue
            # 获取时间戳矩阵对应位置的数据
            dat_mat = [[df[self.idx_of_timesolts[ts]]
                        for ts in ts_seq] for ts_seq in ts_mat]
            x_c, x_p, x_t = np.array(
                dat_mat[0]), np.array(
                dat_mat[1]), np.array(
                dat_mat[2])
            # (t_c, ..., feature_dim), (t_p, ..., feature_dim), (t_t, ..., feature_dim)
            x_exist = [x_ for x_ in [x_c, x_p, x_t] if len(x_) > 0]
            # (t_c+t_p+t_t, ..., feature_dim)
            x_input = np.concatenate(x_exist, axis=0)
            x.append(x_input)
            cur_index = self.idx_of_timesolts[cur_ts]
            # 预测目标即cur_ts处的数据, (1, ..., feature_dim)
            y_input = df[cur_index:cur_index + 1]
            y.append(y_input)
    ...
    def _get_external_array(
        ...
        if self.add_time_in_day:
            time_ind = (
                timestamp_list - timestamp_list.astype("datetime64[D]")) / np.timedelta64(1, "D")
        ...
        if ext_data is not None:
            ...
            # len(timestamp_list) * ext_dim 选出所需要的时间步的数据
    ...
    def _load_data(self):
        ...
        # (num_samples_plus, T_c+T_p+T_t, ..., feature_dim)
    ...
    def _generate_data(self):
        ...
        self._logger.info("x shape: " + str(x.shape) +
                          ", y shape: " + str(y.shape))
        self._logger.info("ext_x shape: " + str(ext_x.shape) +
                          ", ext_y shape: " + str(ext_y.shape))
