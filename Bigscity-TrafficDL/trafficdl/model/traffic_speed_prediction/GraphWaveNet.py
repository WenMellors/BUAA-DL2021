...
class GWNET(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.gcn_bool and self.addaptadj:
            if self.aptinit is None:
                ...
                self.nodevec1 = nn.Parameter(
                    initemb1, requires_grad=True).to(
                    self.device)
                self.nodevec2 = nn.Parameter(
                    initemb2, requires_grad=True).to(
                    self.device)
                self.supports_len += 1
        ...
        for b in range(self.blocks):
            ...
            for i in range(self.layers):
                ...
                additional_scope *= 2
                if self.gcn_bool:
                    self.gconv.append(GCN(self.dilation_channels,
    ...
    def forward(self, batch):
        ...
        if in_len < self.receptive_field:
            x = nn.functional.pad(
                inputs, (self.receptive_field - in_len, 0, 0, 0))
        else:
            x = inputs
