...
class GTS(AbstractTrafficStateModel, Seq2SeqAttrs):
    def __init__(self, config, data_feature, train_data_array):
        ...
        self.cl_decay_steps = int(config.get('cl_decay_steps', 1000))
        self.use_curriculum_learning = bool(config.get('use_curriculum_learning', True))
        self.temperature = 0.5
        self.epoch_use_regularization = int(config.get('epoch_use_regularization'))

        self.num_nodes = self.data_feature.get('num_nodes', 1)  # 网格个数
        self.dim_fc = int(config.get('dim_fc'))
        ...
        df = np.array(train_data_array)
        df = df.reshape((df.size // self.num_nodes, self.num_nodes))[:30000,:]
        scaler = MyStandardScaler(mean=df.mean(), std=df.std())
    ...
    def forward(self, batch, batches_seen=0):
        ...
        # 图结构的推断过程
        x = self.node_feas.transpose(1, 0).view(self.num_nodes, 1, -1)
        x = self.conv1(x)
        x = F.relu(x)
        x = self.bn1(x)
